...
interface ExamCardProps {
  type: "progress" | "completed" | "available";
  exam: Exam;
  language: Language;
  isMobile: boolean;
  ...
  return (
    ...
    >
        ...
        <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-sm text-blue-600 dark:text-blue-300">
              {Math.round(
                (progressStats.totalAnswers / exam.questionCount) * 100,
              )}
              %
            </span>
          </div>
          <div className="w-full bg-blue-200 dark:bg-blue-800 rounded-full h-2">
            <div
              ...
              style={{
                width: `${Math.round((progressStats.totalAnswers / exam.questionCount) * 100)}%`,
              }}
            ></div>
          </div>
          <div className="flex justify-between text-xs text-blue-600 dark:text-blue-300 mt-1">
            <span>
              {progressStats.totalAnswers}/{exam.questionCount}{" "}
              {t("questionsAnswered")}
            </span>
            <span>
              {progressStats.accuracy}% {t("accuracy")}
            </span>
          </div>
      ...
      <div className="flex gap-2">
        {type === "available" && (
          // Case 1: No progress - show Start Exam button
          <button
            onClick={(e) => {
              e.stopPropagation();
              onExamClick(exam);
        ...
        {type !== "available" && hasProgress && (
          // Case 2: Has progress - show Continue Exam button
          <button
            onClick={(e) => {
              e.stopPropagation();
              onExamClick(exam);
